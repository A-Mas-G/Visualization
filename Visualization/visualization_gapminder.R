...
gapminder %>%
  filter(year == 2015 & country %in% c("Sri Lanka", "Turkey", "Colombia")) %>%
...
countries <- c("South Korea", "Germany", "Colombia")
...
labels <- data.frame(country = countries, x = c(1975, 1965, 1970), y = c(60, 72, 65))
gapminder %>% filter(country %in% countries) %>%
